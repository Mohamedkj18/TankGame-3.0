...
ParsedMap AbstractMode::parseBattlefieldFile(const std::string& filename) {
    ...
    if (!file) throw std::runtime_error("Cannot open map file: " + filename);
    ...
    if (!std::getline(file, line)) fail("Missing line 1 (map name/description).");
    chomp_cr(line);

    // Line 2: MaxSteps
    if (!std::getline(file, line)) fail("Missing line 2 (MaxSteps).");
    chomp_cr(line);
    {
        std::regex rgx(R"(MaxSteps\s*=\s*(\d+))");
        std::smatch m; if (!std::regex_search(line, m, rgx)) fail("Bad MaxSteps format.");
        if (m[1].str().empty()) fail("MaxSteps value missing.");
    ...
    // Line 3: NumShells
    if (!std::getline(file, line)) fail("Missing line 3 (NumShells).");
    chomp_cr(line);
    {
        std::regex rgx(R"(NumShells\s*=\s*(\d+))");
        std::smatch m; if (!std::regex_search(line, m, rgx)) fail("Bad NumShells format.");
    }
    parsed.num_shells = std::stoul(std::regex_replace(line, std::regex(R"(.*=\s*)"), ""));

    // Line 4: Rows
    if (!std::getline(file, line)) fail("Missing line 4 (Rows).");
    chomp_cr(line);
    {
        std::regex rgx(R"(Rows\s*=\s*(\d+))");
        std::smatch m; if (!std::regex_search(line, m, rgx)) fail("Bad Rows format.");
    }
    parsed.map_height = std::stoul(std::regex_replace(line, std::regex(R"(.*=\s*)"), ""));
    if (parsed.map_height == 0) fail("Rows must be >= 1.");

    // Line 5: Cols
    if (!std::getline(file, line)) fail("Missing line 5 (Cols).");
    chomp_cr(line);
    {
        std::regex rgx(R"(Cols\s*=\s*(\d+))");
        std::smatch m; if (!std::regex_search(line, m, rgx)) fail("Bad Cols format.");
    }
    parsed.map_width = std::stoul(std::regex_replace(line, std::regex(R"(.*=\s*)"), ""));
    if (parsed.map_width == 0) fail("Cols must be >= 1.");
    ...
    for (size_t row = 0; row < parsed.map_height; ++row) {
        ...
        if (line.size() < parsed.map_width) line.append(parsed.map_width - line.size(), ' ');
        ...
        for (size_t col = 0; col < parsed.map_width; ++col) {
            ...
            switch (c) {
                case '#': parsed.walls.insert({row, col}); break;
                case '@': parsed.mines.insert({row, col}); break;
                case '1': parsed.player1tanks.insert({row, col}); break;
                case '2': parsed.player2tanks.insert({row, col}); break;
                default: /* empty */ break;
            }
        }
    }
